...
These artifacts, whether big or small, won't easily disappear when one resizes the image, or when converting it into a different format. Any additional manipulations will only compound the artifacts that were already left behind, and reduce the overall image quality & fidelity even further over time. This loss in fidelity is usually indicative of a greater distance between the image in question and its original authorship. 
...
That said, ELA is just one of _many_ tools that can be used in forensics. All of them require insight, experience, digital literacy, and an attention to detail in order to be at all worthy of using with any merit or argument. If one doesn't believe an image to be authentic, then the onus is on them to make a strong enough statement for that to be the case. One must have multiple reasons that are supplied with evidence, as well as lack any preconceptions or biases whilst proclaiming their findings. The ability to make informed decisions with our own eyes is more important now than ever in an age where it is so easy to manipulate, misuse, misinterpret, and sell lies, and the truth harder to buy. In an age where it's easier to profit off of lies, the ability to find authenticity in our social feeds is only getting harder all the time. And if the truth becomes too difficult to sell, then what is there even left to buy?
